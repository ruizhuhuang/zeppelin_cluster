...
# export JAVA_HOME=
export MASTER=$SPARK_MASTER_URL                 		# Spark master url. eg. spark://master_addr:7077. Leave empty if you want to use local mode.
...
export ZEPPELIN_LOG_DIR=$Z_LOCAL/logs        		# Where log files are stored.  PWD by default.
export ZEPPELIN_PID_DIR=$Z_LOCAL/run        		# The pid files are stored. ${ZEPPELIN_HOME}/run by default.
export ZEPPELIN_WAR_TEMPDIR=$Z_LOCAL/webapps    		# The location of jetty temporary directory.
export ZEPPELIN_NOTEBOOK_DIR=$Z_LOCAL/notebook   		# Where notebook saved
...
export ZEPPELIN_INTERPRETER_LOCALREPO=$Z_LOCAL/local-repo       # Local repository for interpreter's additional dependency loading
export ZEPPELIN_DEP_LOCALREPO=$Z_LOCAL/local-repo
...
export SPARK_SUBMIT_OPTIONS="--master $MASTER --driver-memory 4g  --executor-memory 2g --num-executors 12 --executor-cores 2 --conf spark.r.command=$TACC_R_BIN/Rscript --conf spark.executor.extraLibraryPath=$LD_LIBRARY_PATH"          # (optional) extra options to pass to spark submit. eg) "--driver-memory 512M --executor-memory 1G".
