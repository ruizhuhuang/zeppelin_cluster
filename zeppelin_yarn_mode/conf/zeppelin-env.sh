...
# export ZEPPELIN_JAVA_OPTS="-Dspark.r.command=$TACC_R_BIN/Rscript -Dspark.executor.extraLibraryPath=$LD_LIBRARY_PATH"      		# Additional jvm options. for example, export ZEPPELIN_JAVA_OPTS="-Dspark.executor.memory=8g -Dspark.cores.max=16"
...
export ZEPPELIN_LOG_DIR=$zeppelin_etc/logs        		# Where log files are stored.  PWD by default.
export ZEPPELIN_PID_DIR=$zeppelin_etc/run        		# The pid files are stored. ${ZEPPELIN_HOME}/run by default.
export ZEPPELIN_WAR_TEMPDIR=$zeppelin_etc/webapps    		# The location of jetty temporary directory.
export ZEPPELIN_NOTEBOOK_DIR=$zeppelin_conf_note/notebook   		# Where notebook saved
...
export ZEPPELIN_INTERPRETER_LOCALREPO=$zeppelin_etc/local-repo         # Local repository for interpreter's additional dependency loading
export ZEPPELIN_DEP_LOCALREPO=$zeppelin_etc/local-repo
...
##
export SPARK_HOME=/data/apps/spark-2.1.0-bin-hadoop2.7/                             # (required) When it is defined, load it instead of Zeppelin embedded Spark libraries
export SPARK_SUBMIT_OPTIONS="--master yarn --deploy-mode client --driver-memory 4g  --executor-memory 3g --num-executors 12 --executor-cores 2 --conf spark.r.command=$TACC_R_BIN/Rscript --conf spark.executor.extraLibraryPath=$LD_LIBRARY_PATH"          # (optional) extra options to pass to spark submit. eg) "--driver-memory 512M --executor-memory 1G".
...
##
# Options read in YARN client mode# export HADOOP_CONF_DIR         		# yarn-site.xml is located in configuration directory in HADOOP_CONF_DIR.
